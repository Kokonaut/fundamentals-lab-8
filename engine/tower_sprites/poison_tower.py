...
class PoisonTowerSprite(TowerSprite):

    def get_tower_base(self):
        image = pyglet.image.load('assets/tower_poison/tower.png')
    ...
    def get_attack_sprite(self):
        attack_frames = [pyglet.resource.image('assets/tower_poison/attack_effect/' + f)
                         for f in get_files_in_path('assets/tower_poison/attack_effect/')]
    ...
    def get_spark_sprite(self):
        image = pyglet.image.load('assets/tower_poison/attack.png')
        image.anchor_x = image.width // 2
